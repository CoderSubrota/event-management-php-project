...
<?php
    ...
    if (isset($_POST['add_attendees'])) {
     ...
     require_once "../back-end/get_user_id.php" ;
        ...
        $get_attendee = "SELECT * FROM attendees WHERE event_id='$event_id'";
        $attendees = mysqli_query($connection, $get_attendee);
        ...
        if ($attendees_row  == 0 ) {
              ...
              $insert_attendee = mysqli_query($connection, $insert);
              if($insert_attendee){
        ...
        if (intval($max_capacity) > intval($attendees_row)) {
            $insert = "INSERT INTO attendees(created_by, full_name, email,event_id) VALUES ('$user_id','$full_name', '$email', '$event_id')";
              ...
              if($insert_attendee){
              header("location: ./dashboard.php");
              $_SESSION["message"] = "Attendee ". $full_name  ." added successfully ";
              exit(); 
            } else {
                ?>
                <script>
...
<div class="container mt-2">
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"
        ...
        <h3 class="text-primary text-center mb-3">Add Attendee</h3>
        ...
        <button type="submit" name="add_attendees" class="btn btn-primary w-100">Save</button>
